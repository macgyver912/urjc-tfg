...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.95\linewidth]{images/content/PBN.png}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.80\linewidth]{images/content/RNAV_RNP.png}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.95\linewidth]{images/content/PBN_diagrama.png}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.80\linewidth]{images/content/GBAS.jpg}
...
La necesidad de mínimas radioayudas en tierra, sólo como sistema de respaldo (\textit{backup}), por lo que no hay que invertir en nuevas. Incluso, serían más accesibles aquellos aeropuertos que no cuenten con sistemas de aterrizaje por instrumentos como ILS o MLS a un coste reducido.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.60\linewidth]{images/content/GNSS.jpg}
	\caption[Constelaciones de satélites para navegación, GNSS.]{Constelaciones de satélites para navegación, GNSS \citeFig{GNSS_url}.}
	\label{fig:GNSS_url}
\end{figure}
...
\begin{figure}[!hbtp]
	...
	\begin{subfigure}{0.5\textwidth}
		\centering
		\includegraphics[height=5.0cm]{images/content/Radar.jpg}
...
En 1944 fue creada la OACI (Organización de Aviación Civil Internacional) o ICAO (\textit{International Civil Aviation Organization}) con el objetivo de regular y organizar el tráfico aéreo creciente, siendo después de la Segunda Guerra Mundial (1939-1945) cuando fue activada \cite{Ontiveros}.

Ésta divide el espacio aéreo mundial en varias zonas:  
Norteamérica (NAM),
África (AFI),
Atlántico Norte (NAT),
Europa (EUR),
...
\begin{itemize}
	...
	\item Espacio aéreo inferior o Región de Información de Vuelo, FIR (\textit{Flight Information Region}). Comprende desde la superficie del terreno (GND, \textit{Ground}) hasta FL245 \footnote{Estos niveles de vuelo umbrales pueden variar según necesidades de cada espacio aéreo.}.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.85\linewidth]{images/content/Sectores.jpg}
	\caption[Sectorización de las zonas de control en España.]{Sectorización de las zonas de control en España \citeFig{Insignia_url}.}
	\label{fig:Sectores}
\end{figure}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.75\linewidth]{images/content/AreasATC.jpg}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.45\linewidth]{images/content/ZonasDRP.jpg}
...
\begin{itemize}
	...
	\item Horizontal: se separan de manera longitudinal -unas detrás de otras por una distancia o tiempo marcados- o bien lateral, por una distancia regulada. 
%Se asigna en base a la información de su posición proporcionada al controlador aéreo por los pilotos, o bien con la ayuda del radar al proporcionar este sistema información sobre la posición de las aeronaves. 
Por tanto, depende de la fase del vuelo (despegue, en ruta, aproximación, aterrizaje), de la estela turbulenta del avión precedente así como de la precisión de posicionamiento y las velocidades relativas entre ambas aeronaves. Varía entre \SI{3}{} y \SI{10}{NM}.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.7\linewidth]{images/content/RVSM.jpg}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.7\linewidth]{images/content/UTC.jpg}
	\caption[Husos horarios y definición de UTC.]{Husos horarios y definición de UTC \citeFig{UTC_url}.}
	\label{fig:UTC}
\end{figure}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.80\linewidth]{images/content/TCAS.jpg}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=1.0\linewidth]{images/content/LEBL_Convencional.png}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=1.0\linewidth]{images/content/LEBL_P-RNAV.png}
...
Se trata de un simulador de control de tráfico aéreo basado en navegadores web en el que el usuario actúa como controlador de aproximación para guiar a las aeronaves en salida y llegada al aeropuerto asegurando una separación mínima lateral de \SI{3}{NM} y vertical de \SI{1000}{ft} \cite{ATC-SIM}.

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.90\linewidth]{images/simulators/ATC-SIM.jpg}
	\caption[Captura de pantalla del simulador ATC-SIM en Chicago O'Hare (KORD/ORD).]{Captura de pantalla del simulador ATC-SIM en Chicago O'Hare (KORD/ORD).}
	\label{fig:ATC-SIM}
\end{figure}

Los comandos de instrucción se realizan mediante texto, seleccionando el tráfico a instruir y escribiendo el carácter del tipo de instrucción:

\begin{itemize}
\item 'c' o 'C' para (\textit{cleared to}) altitud seguido de uno o dos dígitos (miles de pies), rumbo con tres dígitos (rellenando con ceros por la izquierda) o volar directo a un punto.

\item 's' o 'S' (\textit{speed}) para velocidad seguido de la velocidad (superior a \SI{140}{kt}).

\item 'l' o 'L' (\textit{landing}) para autorización a aterrizar seguido de la pista de aterrizaje.

\item 't' o 'T' (\textit{takeoff}) para autorización de despegue. Este comando no tiene parámetro.

\item 'a' o 'A' (\textit{abort}) para abortar un despegue o aterrizaje. Este comando no tiene asociado.

\item 'w' o 'W' (\textit{line and wait}) para alinear y mantener. Este comando no tiene parámetro.

\item 'h' o 'H' (\textit{hold}) para realizar una espera en el punto de ruta que se le pase como parámetro.
\end{itemize}

También tiene modificadores como 'X' (\textit{expedite}) para aumentar la velocidad vertical durante una autorización de altitud y 'L' o 'R' para obligar a realizar un viraje hacia el lado izquierdo o derecho, respectivamente.

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.70\linewidth]{images/simulators/ATC-SIM_label.jpg}
	\caption[Detalle de la etiqueta de un tráfico despegando en EGLL.]{Detalle de la etiqueta de un tráfico despegando en EGLL.}
	\label{fig:SIM_label}
\end{figure}
...
\section{Global ATC Simulator}
\label{sec:ATC-Simulator-Aerosoft}

Se trata de una aplicación que se instala y ejecuta en una máquina local y permite realizar las tareas de los controladores de Torre, Aproximación y Salidas. Las principales características que lo diferencian del resto de simuladores ATC es que incluye la inmensa mayoría de aeropuertos del mundo con sus respectivos procedimientos de salida y llegada. Además, permite personalizar los ficheros para modificar o añadir aeronaves y aerolíneas \cite{ATC-Simulator-Aerosoft}.

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.80\linewidth]{images/simulators/GATC.jpg}
	\caption[Imagen del simulador Global ATC Simulator en Munich (EEDM/MUC).]{Imagen del simulador Global ATC Simulator en Munich (EEDM/MUC).}
	\label{fig:ATC-GATC}
\end{figure}

La interfaz de control puede ser mediante texto o ratón, tal y como se muestra en la Figura \ref{fig:ATC-GATC_input}.

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.50\linewidth]{images/simulators/GATC_input.jpg}
	\caption[Ventana de control del simulador Global ATC Simulator.]{Ventana de control del simulador Global ATC Simulator.}
	\label{fig:ATC-GATC_input}
\end{figure}

Otros puntos interesantes son la simulación de la meteorología real (con conexión a Internet), emergencias por nivel bajo de combustible y frustradas por visibilidad, así como la capacidad de actualizar la base de datos que emplea como base, \textit{NavDataPro}. Esta base de datos ha dejado de ser comercializada por Aerosoft y ahora debe emplearse la de Navigraph, también bajo licencia, empleada por algunos simuladores de vuelo de ámbito semiprofesional y tiene un coste de suscripción de \SI{10.95}{}€ mensuales o \SI{98.78}{}€ anuales \cite{Navigraph}.

Actualmente este simulador tiene un coste de \SI{30.45}{}€ y es compatible con los Sistemas Operativos Windows 7 y Windows 8, suponiéndose compatibilidad para versiones siguientes.


\clearpage

%https://esstudio.site/atc-manager-2/
\section{ATC Manager 2}
\label{sec:ATCManager2}

Este aplicación web ofrece la simulación de un puesto de control unificado de torre, aproximación y ruta, con un sólo aeropuerto y funcionalidades muy similares a las de \textit{\ref{sec:ATC_SIM} \nameref{sec:ATC_SIM}}. Así, se muestran puntos de ruta, ya sean fijos o radioayudas, pero en realidad no hay distinción en el simulador más allá de los nombres, los cuales parecen ficticios, al igual que todo el escenario.

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.90\linewidth]{images/simulators/ATC-Manager.jpg}
	\caption[Imagen del simulador ATC Manager 2.]{Imagen del simulador ATC Manager 2.}
	\label{fig:ATC-Manager}
\end{figure}

En este caso el control de las aeronaves se realiza mediante el cuadro de control de la esquina inferior derecha o bien, mediante texto. Estos comandos por texto son de tipo completo \cite{ATC-Manager}:

\begin{itemize}
\item 'HDG', 'turn' o 'turn left/right heading' seguido del rumbo en grados.

\item 'alt', 'climb and maintain' o 'descend and maintain' seguido de la altitud en pies.

\item 'dct', 'direct' o 'direct to' seguido del punto de ruta o pista a instruir.

\item 'spd', 'increase speed (+to)', 'reduce speed (+to)' seguido de la velocidad en nudos.

\item 'takeoff' o 'clear for takeoff' para autorizar despegue. No tiene parámetro asociado.

\item 'go around' o 'goaround' para solicitar un motor y al aire. No tiene parámetro asociado.
\end{itemize}


%\begin{figure}[!hbtp]
%	\centering
%	\includegraphics[width=0.35\linewidth]{images/simulators/ATC-Manager-Input.jpg}
%	\caption[Ventana de control y comandos del simulador ATC Manager 2.]{Ventana de control y comandos del simulador ATC Manager 2.}
%	\label{fig:ATC-Manager-Input}
%\end{figure}

Una de las características principales de este simulador es la posibilidad de editar aviones y compañías aéreas mediante un formulario que genera un fichero con extensión \texttt{.json}, pudiendo crearse una solicitud de aprobación por el autor para añadirlo al repositorio GitHub del proyecto \cite{ATC-Manager-Git}.


\clearpage

%https://www.openscope.co/
\section{openScope Air Traffic Control Simulator}
\label{sec:openScopeAirTrafficControlSimulator}

Se trata de un simulador basado en navegadores web, actualmente en su versión v6.28.0, que destaca por añadir más posibilidades que los simuladores vistos anteriormente. A las tradicionales funcionalidades de rumbo, altitud, velocidad y volar a un punto directo que comparten todos ellos, añade la simulación de vuelo de rutas SID y STAR, dibujado de áreas restringidas, así como solicitar reportes de estado a los pilotos como altitud y velocidad actuales \cite{ATC-openScopeATC}.

Como puede apreciarse en la Figura \ref{fig:ATC-openScopeATC}, se trata del simulador más completo de todos y también el más complejo de usar por la cantidad de comandos que soporta, siendo tan diversos que merece la pena revisarlos en su página oficial de GitHub \cite{ATC-openScopeATC-Git}, a la que redirige desde la ayuda del simulador. No obstante, en la Tabla \ref{tab:ResumenSimuladores} se muestran todas las funcionalidades.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.99\linewidth]{images/simulators/openScopeATC.jpg}
	\caption[Imagen del simulador openScope Air Traffic Control Simulator.]{Imagen del simulador openScope Air Traffic Control Simulator.}
	\label{fig:ATC-openScopeATC}
\end{figure}

\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.50\linewidth]{images/simulators/openScopeATC_label.jpg}
	\caption[Detalle de las etiquetas de openScope Air Traffic Control Simulator.]{Detalle de las etiquetas de openScope Air Traffic Control Simulator.}
	\label{fig:ATC-openScopeATC_label}
\end{figure}
...
\section{Resumen de los simuladores existentes}
\label{sec:ResumenSimuladores}
...
\begin{table}[!hbtp]
	...
	\begin{center}
		\caption{Síntesis de funcionalidades de los simuladores actuales.}
		\label{tab:ResumenSimuladores}
		\hspace*{-2cm}
		\begin{tabular}{|r|c c c c|c|l|}
			\hline % Title
			\rule[-1ex]{0pt}{2.5ex} 
			\diagbox[innerwidth=\textwidth*1/5]{Función}{Simulador}
			& $\;\;\;\;$\textbf{ATC-SIM}$\;\;\;\;$
			& \begin{tabular}{@{}c@{}}
				\textbf{Global ATC} \\ 
				\textbf{Simulator}
			...
			& \begin{tabular}{@{}c@{}}
				\textbf{ATC Analysis} \\ 
				\textbf{and Training Tool}
			  \end{tabular}
			& \diagbox[dir=NE,innerwidth=\textwidth*1/5]{\textit{Simulator}}{\textit{Feature}}
			\\ 
%			% Test column width
%			\hline 
%			\rule[-1ex]{0pt}{2.5ex}  
...
%			\\ 
			...
			& {\color{olive}\cmark} 
			& {\color{olive}\cmark}
			& \cellcolor{gray!10}{\color{olive}\cmark}
			& {\color{gray}\textit{Heading}}
			\\ 
			%\hline 
			\rule[-1ex]{0pt}{2.5ex} 
			Altitud
			...
			& {\color{red}\xmark} 
			& {\color{olive}\cmark}
			& \cellcolor{gray!10}{\color{blue}\todomark} 
			& {\color{gray}\textit{Holding patterns}}
			\\ 
			%\hline 
			\rule[-1ex]{0pt}{2.5ex} 
			Aterrizajes  
			...
			& {\color{olive}\cmark} 
			& {\color{olive}\cmark}
			& \cellcolor{gray!10}{\color{orange}Transf. TWR} 
			& {\color{gray}\textit{Landings}}
			\\ 
			%\hline 
			\rule[-1ex]{0pt}{2.5ex} 
			Despegues  
			...
			& {\color{olive}\cmark} 
			& {\color{olive}\cmark}
			& \cellcolor{gray!10}{\color{orange}Transf. CTR}
			& {\color{gray}\textit{Takeoffs}}
			\\ 
			\hline 
			\rule[-1ex]{0pt}{2.5ex} 
			Meteorología  
			...
			& {\color{olive}\cmark}
			& {\color{olive}\cmark} 
			& \cellcolor{gray!10}{\color{orange}\readymark} 
			& {\color{gray}\textit{Text To Speech}}
			\\ 
			%\hline 
			\rule[-1ex]{0pt}{2.5ex}
			Zonas restringidas 
			...
			& {\color{olive}\cmark}
			& \cellcolor{gray!10}{\color{blue}\todomark}
			& {\color{gray}\textit{Restricted areas}} 
			\\ 
			\hline 
%			\rule[-1ex]{0pt}{2.5ex}
%			Rumbo  
...
%			& • 
%			& • 
%			& \cellcolor{gray!10}{\color{gray}\textit{Heading}}
%			\\ 
%			\hline 
		\end{tabular}
	\end{center}
...
\begin{center}
{\color{blue}\todomark} Trabajos futuros.
\hspace{2cm}
{\color{orange}\readymark} Preparado para insertar API TTS.
