...
Así, se define la navegación de área o RNAV (\textit{aRea NAVigation}), cuyos fijos de ruta no tienen por qué venir determinados por la cobertura de equipos en tierra sino que pueden ser, por ejemplo, coordenadas latitud-longitud.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.75\linewidth]{images/content/PBN.png}
...
%
%\begin{itemize}
%\item RNAV Básica (B-RNAV) y RNP-5 RNAV ($\pm5$ NM; 95\%).
%\item RNAV Precisión (P-RNAV) y RNP-1 RNAV ($\pm1$ NM; 95\%).
%\end{itemize}
%
%Cada vez más aeropuertos de la geografía española están \textbf{adoptando los procedimientos RNAV}, de momento para salidas por ser menos restrictivas, como son el aeropuerto de Adolfo Suárez - Madrid Barajas, Barcelona - El Prat y Palma de Mallorca. En la siguiente página se muestran a modo de comparativa la misma región en la cartas aeronáuticas para Salidas Instrumentales Normalizadas o SID (\textit{Standard Instrumental Departure}) con navegación convencional y RNAV del aeropuerto de Barcelona - El Prat, en concreto para la pista 25R.
%
%\begin{figure}[htbp]
%	\centering
%		\includegraphics[width=15cm]{./Figuras/LEBL_Convencional.png}
%	\caption{Extracto de SID convencional de la RWY 25R de LEBL (extraído de \cite{ENAIRE_AIP}).}
%	\label{fig:LEBL_Convencional}
%\end{figure}
%
%
%
%Puede aprecierse cómo en la Figura \ref{fig:LEBL_Convencional} todas las salidas hacia Europa y las Islas Baleares cuelgan de un arco definido completamente en base a \textbf{distancias DME de los DVOR} de Barcelona (116.70 MHz) y El Prat (114.30 MHz). Esta navegación se realiza a partir de los \textbf{equipos de a bordo} de la aeronave con ayuda del procesamiento de la señal de las radio ayudas terrestres.
%
%\pagebreak
%
%En cambio, en la Figura \ref{fig:LEBL_RNAV} algunas de las salidas son completamente \textbf{independientes} al resto. Esto se traduce en la posibilidad de realizar tramos más \textbf{directos y cortos}, con el impacto directo sobre el consumo de combustible, las emisiones y el tiempo y la eficiencia de vuelo. Además, contribuye a \textbf{aumentar la capacidad del espacio aéreo}, ya que al estar en salidas con trayectos independientes, pueden sacarse más aviones del aeropuerto sin que uno tenga que ser necesariamente precedente del otro en la misma ruta de salida.
%Por tanto, la capacidad en \textbf{número de operaciones} del aeropuerto se ve \textbf{incrementada}.
%
%\begin{figure}[htbp]
%	\centering
%		\includegraphics[width=15cm]{./Figuras/LEBL_P-RNAV.png}
%	\caption{Extracto de SID P-RNAV de la RWY 25R de LEBL (extraído de \cite{ENAIRE_AIP}).}
%	\label{fig:LEBL_RNAV}
%\end{figure}
...
\item Disponibilidad o probabilidad de que el sistema esté operando con todos sus parámetros dentro de valores admisibles de precisión, integridad y continuidad, durante la operación a realizar. Se expresa como un porcentaje de tiempo, evaludado sobre largos periodos temporales.
...
El problema de estos sistemas embarcados es que una vez se ajustan en tierra, en una posición bien conocida del aeropuerto de origen, no tienen ninguna referencia de otros sistemas a lo largo de todo el trayecto, por lo que el error es acumulativo, haciendo que la precisión en el seguimiento de la ruta sea cada vez menor cuanto mayor sea el tiempo de vuelo. Esto exige que las rutas oceánicas tengan separaciones laterales de varias decenas de millas náuticas.
...
%Uno de los sistemas que está ampliamente extendido a día de hoy y que será obligatorio en los próximos años para todas las aeronaves civiles es el ADS o Vigilancia Dependiente Automática (\textit{Automatic Dependent Surveillance}).
%
%Se trata de un sistema de vigilancia en el cual la aeronave transmite por enlace de datos su \textbf{identificativo}, \textbf{posición} extraída principalmente de los sistemas GNSS de a bordo y otros \textbf{datos adicionales} de relevancia para el personal del Servicio de Tránsito Aéreo. 
%
%Existen dos tipos: \textbf{ADS-C}, donde la transimisión de datos se realiza hacia una estación de tierra previo \textbf{contrato}, y \textbf{ADS-B}, donde la transmisión de datos se realiza mediante \textbf{difusión} o \textit{broadcast}, por lo que cualquier receptor apropiado puede decodificar los datos enviados.
%
%\begin{figure}[htbp]
%	\centering
%		\includegraphics[width=14cm]{./Figuras/Flightradar24.png}
%	\caption{Datos ADS-B recogidos por la plataforma Flightradar24 (\cite{ENAIRE_AIP}).}
%	\label{fig:Flightradar24}
%\end{figure}
%
%Como se comentó anteriormente, al igual que ocurría con la navegación en rutas oceánicas, la \textbf{vigilancia} del espacio aéreo sobre el océano es \textbf{muy limitada} y prácticamente se reduce a comunicaciones de voz en banda HF entre piloto y controlador para reportar posición y coordenadas y hora del siguiente punto de la ruta.
%
%Con \textbf{ADS} se puede tener una \textbf{mejor cobertura mundial} sobre zonas donde los radares primaro (PSR) y secundario (SSR) no alcanzan, además de tener muchas otras ventajas que pueden incorporarse a futuros sistemas tanto de vigilancia como de comunicaciones y navegación.
%
%\section{Conclusiones}
%\label{sec:Conclusiones}
%
%Como se ha detallado a lo largo del documento, \textbf{PBN} supone una \textbf{mejora en la calidad del servicio} prestado realizando un cambio de ideología sobre los procedimientos convencionales. Esto repercute directamente en una mejor protección del \textbf{medio ambiente}, una reducción de \textbf{costes} para las compañías aéreas al evitar frustradas y desvíos a alternativos, sobre todo en aeropuertos con pocas infraestructuras de radio ayudas, y como no podría ser de otro modo en este sector, una mejora la \textbf{seguridad} en las operaciones.
%
%Hay que destacar que PBN se basa en la \textbf{navegación GNSS}, y es ésta tecnología la que ha posibilidado, a través de \textbf{sistemas de aumentación}, tener sistemas confiables y seguros a nivel global, además de ser uno de los facilitadores para otro tipo de funcionalidades que vendrán en el futuro.
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.85\linewidth]{images/content/ReglasVuelo.jpg}
...
\begin{figure}[!hbtp]
	\centering
	\includegraphics[width=0.9\linewidth]{images/content/RVSM.jpg}
...
//TODO: rutas en ruta, carta aeronautica AIP

\subsection{Salidas normalizadas por instrumentos}
\label{subsec:SID}

//TODO: SID del AIP, convencional vs RNAV

\subsection{Llegadas normalizadas por instrumentos}
\label{subsec:STAR}

//TODO: STAR del AIP, convencional vs RNAV
